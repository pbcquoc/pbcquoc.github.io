...
katex.render("\\sum_{i=0}^{S^{2}}\\sum_{j=0}^{B}objindicator_{ij}^{obj}(C_{i}-\\hat{C_{i}})^{2} + \\lambda_{noobject} \\sum_{i=0}^{S^{2}}\\sum_{j=0}^{B}noobjindicator_{ij}^{obj}(C_{i}-\\hat{C_{i}})^{2}", confidence_loss[0]);
</script>
